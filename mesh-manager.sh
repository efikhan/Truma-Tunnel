...
#   - Network secret moved to EnvironmentFile (not exposed in ps)
#   - Added iptables persistence (save rules)
...
# Guard clause
if [[ "${__mesh_manager_loaded:-}" == "true" ]]; then
    return 0
fi
__mesh_manager_loaded=true
...
if ! declare -f valid_port >/dev/null 2>&1; then
    valid_port() { is_int "$1" && (( 10#$1 >= 1 && 10#$1 <= 65535 )); }
fi
...
# -----------------------------------------------------------------------------
# Disk space check (if not defined in truma.sh)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Improved input functions (with printf -v, no eval)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Improved network validation (allows all private ranges)
# -----------------------------------------------------------------------------
valid_base_network() {
    local net="$1"
...
# -----------------------------------------------------------------------------
# Install EasyTier core with checksum verification and disk space check
# -----------------------------------------------------------------------------
mesh::install_core() {
    ...
    # Check disk space (approx 50MB needed)
    check_disk_space 50 || return 1

    # Use temporary file and check size
    ...
    # Optional checksum verification (if we have known hashes)
    # For production, use actual checksums from a trusted source
    # if [[ -n "$expected_cli_sha256" ]]; then
    #     actual_cli_sha256=$(sha256sum "$temp_cli" | awk '{print $1}')
...
# -----------------------------------------------------------------------------
# Create config directories and secret file
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Main creation function (with improved validation)
# -----------------------------------------------------------------------------
mesh::create_interactive() {
    ...
    # Improved side selection with loop
    while true; do
        echo "Select side:"
        echo "  1) Direct"
        echo "  2) Remote"
...
# -----------------------------------------------------------------------------
# Peers and routes (unchanged but with better error handling)
# -----------------------------------------------------------------------------
mesh::list_peers() {
    local name="$1"
...
# -----------------------------------------------------------------------------
# Improved secret retrieval (using env file)
# -----------------------------------------------------------------------------
mesh::show_secret() {
    local name="$1"
    local env_file="$MESH_SECRET_DIR/mesh-${name}.env"
...
# -----------------------------------------------------------------------------
# Improved removal with better error handling
# -----------------------------------------------------------------------------
mesh::remove() {
    local name="$1"
