...
if ! declare -f valid_base_network >/dev/null 2>&1; then
    valid_base_network() {
        ...
        # Must be 10.x.y.0 and middle octets between 0-255 (already checked by valid_ipv4)
        [[ "$a" == "10" && "$d" == "0" ]] || return 1
        return 0
    }
fi
...
# -----------------------------------------------------------------------------
# Placeholder functions for compatibility (with improved messages)
# -----------------------------------------------------------------------------
mesh::list_ports() {
...
mesh::add_port_interactive() {
    ...
    print_info "Please use the Port Management menu to add HAProxy rules for this tunnel."
    pause_enter
}
