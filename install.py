...
- If Iran is selected:
    1. Disable IPv6 immediately.
    2. Install dependencies.
    3. Test and select best DNS & mirror, apply kernel tuning.
- Handles GitHub access issues by falling back to direct file download via curl.
...
FALLBACK_BASE_URL = "https://raw.githubusercontent.com/efikhan/Truma-Tunnel/main"
MIRROR_BASE_URL = "https://hub.fastgit.xyz/efikhan/Truma-Tunnel/raw/main"  # mirror جایگزین
FILES = [
    "truma.sh",
    "gre-manager.sh",
    "paqet.sh",
...
# -------------------- Iran Optimizations --------------------
def disable_ipv6():
    """Disable IPv6 immediately, before any network operations."""
    print_step("Disabling IPv6 (first step for Iran)...")
...
def run_iran_optimizations_after_deps():
    """Run DNS, mirror, and kernel optimizations after dependencies are installed."""
    select_best_dns()
    select_best_mirror()
    apply_kernel_tuning()
    print_success("Iran optimizations completed (DNS, mirror, kernel).")
...
# -------------------- Truma Installation with Fallback --------------------
def download_files_via_curl(base_url, dest_dir):
    """Download all required files using curl from a given base URL."""
    print_step(f"Downloading files from {base_url} ...")
    for filename in FILES:
        url = f"{base_url}/{filename}"
        dest = os.path.join(dest_dir, filename)
        print_info(f"  Downloading {filename} ...")
        # Use curl with --fail to detect HTTP errors
        proc = run_cmd(["curl", "-fL", "--max-time", "30", "-o", dest, url], check=False, capture=True)
        if proc.returncode != 0:
            print_warning(f"Failed to download {filename} from {base_url}")
            return False
    return True
...
def install_truma():
    ...
    git_success = False
    if shutil.which("git"):
        print_step("Attempting to clone repository via git...")
        with tempfile.TemporaryDirectory() as tmpdir:
            proc = run_cmd(["git", "clone", "--depth", "1", REPO_URL, tmpdir], check=False, capture=True)
            if proc.returncode == 0:
                ...
                for item in os.listdir(tmpdir):
                    ...
                    if os.path.isdir(s):
                        ...
                        shutil.copy2(s, d)
                print_success("Repository cloned successfully via git.")
                git_success = True
            else:
                print_warning("git clone failed; will try direct download.")

    # If git failed, try direct download from raw.githubusercontent.com
    if not git_success:
        print_step("Attempting direct download from GitHub raw...")
        success = download_files_via_curl(FALLBACK_BASE_URL, INSTALL_DIR)
        if not success:
            print_warning("Download from raw.githubusercontent.com failed; trying mirror...")
            success = download_files_via_curl(MIRROR_BASE_URL, INSTALL_DIR)
        if not success:
...
def main():
    ...
    # Step 2: If inside Iran, disable IPv6 immediately (before any network ops)
    if choice == "1":
        disable_ipv6()

    # Step 3: Install dependencies (needed for both optimization and installation)
    install_dependencies()

    # Step 4: If inside Iran, run remaining optimizations (DNS, mirror, kernel)
    if choice == "1":
        run_iran_optimizations_after_deps()
    else:
        print_info("Server is outside Iran. Skipping optimizations.")

    # Step 5: Install Truma
