...
#   [2] Sanitized description to prevent newline injection
#   [3] Set strict file permissions (umask 077, chmod 600)
#   [4] Added port conflict check (system-wide and existing rules)
#   [5] Improved port extraction using basename + sed (more robust)
#   [6] Added multi-distro package manager support (apt, yum, dnf, pacman, apk)
#   [7] Improved HAProxy reload/start logic with error handling
#   [8] Added checksum verification placeholder (for future use)
#   [9] Made file paths configurable via environment variables
# =============================================================================
...
# [FIX #1] Use printf -v instead of eval
...
# -----------------------------------------------------------------------------
# [FIX #9] Configurable paths (with defaults)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# [FIX #6] Multi-distro package manager support
# -----------------------------------------------------------------------------
install_haproxy() {
...
# -----------------------------------------------------------------------------
# [FIX #4] Port conflict detection (system-wide)
# -----------------------------------------------------------------------------
...
haproxy::install() {
    if ! command -v haproxy &>/dev/null; then
        ...
        # [FIX #8] Checksum verification (placeholder)
        # if command -v sha256sum &>/dev/null; then
        #     actual_hash=$(sha256sum "$(command -v haproxy)" | awk '{print $1}')
        #     if [[ "$actual_hash" != "$expected_hash" ]]; then
        #         print_error "Checksum mismatch! Possible compromise."
        #         return 1
        #     fi
        # fi
...
haproxy::add_rule() {
    ...
    # [FIX #4] Check port availability (unless overwriting the same file)
    ...
    # [FIX #2] Sanitize description: remove newlines and carriage returns
    local desc_safe
    desc_safe="${desc//$'\n'/}"
    desc_safe="${desc_safe//$'\r'/}"
    # Remove any '#' characters to avoid comment injection (though they'd be harmless after #)
    desc_safe="${desc_safe//#/}"

    # [FIX #3] Create file with secure permissions
    umask 077
    {
...
haproxy::list_rules() {
    ...
    for f in "$HAPROXY_CONFD"/rule_*.cfg; do
        [[ -f "$f" ]] || continue
        count=$((count+1))
        # [FIX #5] Use basename and sed, but also check with pattern
        local filename port
        filename=$(basename "$f")
        port=$(sed 's/^rule_//;s/\.cfg$//' <<<"$filename")
        # Ensure port is numeric
        if ! valid_port "$port"; then
            print_warning "Skipping file with invalid port name: $filename"
            continue
        fi
        local target
...
haproxy::apply() {
    ...
    if haproxy -c -f "$HAPROXY_CFG" -f "$HAPROXY_CONFD" &>/dev/null; then
        if command -v systemctl &>/dev/null; then
            # [FIX #7] Improved reload/start logic
