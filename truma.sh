...
# Source modules
for f in gre-manager.sh paqet.sh mesh-manager.sh haproxy-manager.sh; do
...
_ensure_cmd() {
    ...
    # In non-interactive mode, we already handled above; here we are interactive.
...
# Using sed instead of grep -oP for compatibility
# =============================================================================
get_target_ip_for_tunnel() {
    ...
    if [[ "$type" == "gre" ]]; then
        ...
        if [[ -f "$service_file" ]]; then
            # Extract local_ip using sed instead of grep -oP
            local local_ip
            local_ip=$(sed -n 's/.*-i \([0-9.]\+\).*/\1/p' "$service_file" | head -1)
            if [[ -n "$local_ip" ]]; then
                target_ip=$(echo "$local_ip" | awk -F. '{print $1"."$2"."$3"."($4==1?2:1)}')
            fi
        fi
    fi
    echo "$target_ip"
...
create_tunnel() {
    ...
    [[ "$tunnel_type" != "1" && "$tunnel_type" != "2" && "$tunnel_type" != "3" ]] && { add_log "Invalid choice."; pause_enter; return 0; }
...
# =============================================================================
# Port management functions (using HAProxy)
# =============================================================================
add_tunnel_port() {
    render
...
# =============================================================================
# Complete tunnel uninstall (including HAProxy rules)
# =============================================================================
uninstall_clean() {
    render
    ...
    # Remove tunnel service using protocol-specific function
    if [[ "$type" == "gre" ]]; then
        gre::remove "$name"
    elif [[ "$type" == "paqet" ]]; then
        paqet::remove "$name"
