...
#   [1] Removed dangerous `sudo -E` (privilege escalation risk)
#   [2] Replaced all `eval` with `printf -v` (shell injection fix)
#   [3] Improved input validation (sanitization and retry limits)
#   [4] Enhanced error handling (removed unnecessary `|| true`)
#   [5] Port conflict check integrated with HAProxy manager
# =============================================================================
...
# =============================================================================
# Root check (fixed: removed -E flag)
# =============================================================================
ensure_root() {
    if [[ $EUID -ne 0 ]]; then
        if command -v sudo &>/dev/null; then
            echo -e "${YELLOW}[!] This script must be run as root. Re‑running with sudo...${NC}"
            exec sudo bash "$0" "$@"   # Removed -E to prevent environment variable leakage
        else
...
# =============================================================================
check_dependencies() {
    local deps=("curl" "systemctl" "iptables" "sed" "grep" "ip" "openssl")
    local missing=()
    for cmd in "${deps[@]}"; do
...
# Helper to source a module with error handling
source_module() {
    local module="$1"
...
# =============================================================================
# UI helpers (unchanged but included for completeness)
# =============================================================================
declare -a LOG_LINES=()
LOG_MIN=3
LOG_MAX=10
...
# =============================================================================
# Input validation functions (with sanitization)
# =============================================================================
...
# =============================================================================
# Tunnel name retrieval functions (unchanged)
# =============================================================================
get_tunnel_names() {
    local names=()
