...
#   [1] Safe DNS configuration with systemd-resolved detection
#   [2] Added checksum verification for cloned repository
#   [3] Backup before destructive operations (rm -rf)
#   [4] Fixed mirror test for Debian/Ubuntu
#   [5] Added --fail to curl to avoid false positives
#   [6] Improved sources.list replacement with comprehensive regex
#   [7] Prevented duplicate sysctl entries (replace instead of append)
#   [8] Replaced recursion in menu with a while loop
# =============================================================================
...
# -----------------------------------------------------------------------------
# [FIX #7] Sysctl configuration (replace, not append)
# -----------------------------------------------------------------------------
set_sysctl_param() {
    local key="$1"
    local value="$2"
...
# [FIX #1] Safe DNS configuration (respect systemd-resolved)
select_best_dns() {
...
nameserver 1.1.1.1
EOF
            # Only lock if not using resolved (to avoid conflicts)
            chattr +i /etc/resolv.conf 2>/dev/null || true
...
# [FIX #4] Mirror test with correct distribution paths
select_best_mirror() {
    ...
    # Fallback codename if not detected
    if [[ -z "$codename" ]]; then
        if command -v lsb_release &>/dev/null; then
            codename=$(lsb_release -cs)
        else
            codename="noble"  # fallback for Ubuntu 24.04
        fi
    fi
    ...
    case "$distro" in
        ...
        centos|rhel|almalinux|rocky)
            ...
            ;;
        *)
            test_path="ubuntu/dists/noble/InRelease"  # fallback
            ;;
    esac
    ...
    for entry in "${mirrors[@]}"; do
        ...
        # Construct proper URL based on distro
        local url=""
        case "$distro" in
            ubuntu|debian)
                ...
                ;;
            centos|rhel|almalinux|rocky)
                url="http://$mirror/centos/$test_path"  # adjust if needed
                ;;
            *)
                url="http://$mirror/ubuntu/$test_path"
                ;;
        esac
        # [FIX #5] Use --fail to avoid false positives
        local speed
    ...
    if [[ -n "$best_mirror" ]]; then
        ...
        # [FIX #6] Comprehensive sources.list update
        if [[ -f /etc/os-release ]]; then
            . /etc/os-release
            case "$ID" in
                ubuntu|debian)
                    if [[ -f /etc/apt/sources.list ]]; then
                        ...
                        # Replace any ubuntu/debian mirror
...
install_truma() {
    ...
    # [FIX #3] Backup existing installation
    ...
    # [FIX #2] Checksum verification (optional, but recommended)
    # If a checksums file exists in the repo, verify the main script
...
# -----------------------------------------------------------------------------
# [FIX #8] Main menu with while loop instead of recursion
# -----------------------------------------------------------------------------
main_menu() {
    local choice
    while true; do
