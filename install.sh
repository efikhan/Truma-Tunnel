...
EOF
    ...
    local msg="Please select your server location:\n\n   1) Inside Iran\n   2) Outside Iran\n\n   (Default: 1)"
    render_box "Server Location" "$msg"
    echo
    echo -ne "${YELLOW}Choice (1 or 2) [1]: ${NC}"
    read -r choice
    choice="${choice:-1}"
    ...
    if [[ "$choice" == "1" ]]; then
        ...
        print_info "Server is outside Iran. Only Truma will be installed."
        IRAN_OPTIMIZE=0
    fi
    echo
    pause_enter
