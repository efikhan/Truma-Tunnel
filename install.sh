...
disable_ipv6() {
    ...
    # Safe here-doc without || problems
    if ! cat > /etc/sysctl.d/99-ipv6-disable.conf <<'EOF' 2>/dev/null; then
...
select_best_dns() {
    ...
    if systemctl is-active systemd-resolved &>/dev/null; then
        ...
        # Use separate here-doc without if embedding
        cat > /etc/systemd/resolved.conf <<'EOF' 2>/dev/null
[Resolve]
...
EOF
    if [[ $? -ne 0 ]]; then
        print_warning "Could not write kernel tunings."
    fi
