...
if ! command -v wget &>/dev/null; then
    ...
    if command -v apt-get &>/dev/null; then
        ...
        echo -e "${RED}No supported package manager. Please install wget manually.${NC}"
        exit 1
    fi
fi
...
# Auto-start Truma (this will wait until Truma exits)
if [[ -f "./truma.sh" ]]; then
    ./truma.sh
else
    echo -e "${RED}Error: truma.sh not found!${NC}"
    exit 1
fi

echo -e "${GREEN}Truma exited. Installation script finished.${NC}"
exit 0
