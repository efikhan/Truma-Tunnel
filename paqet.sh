...
#   - Added iptables persistence via iptables-save in service scripts
...
# Guard clause
if [[ "${__paqet_manager_loaded:-}" == "true" ]]; then
    return 0
fi
__paqet_manager_loaded=true
...
# [FIX #1, #7, #8] All read functions use printf -v instead of eval, and include validation
...
# [FIX #2] Added checksum verification and disk space check
paqet::install() {
...
paqet::create_interactive() {
    ...
    # Save iptables rules after successful start
    save_iptables_rules
