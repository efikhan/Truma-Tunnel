...
# -----------------------------------------------------------------------------
# Port management functions (uses config file YAML)
# -----------------------------------------------------------------------------
paqet::add_ports() {
    local name="$1"; shift
    local -a new_ports=("$@")
    ...
    # Extract existing ports using sed instead of grep -oP
    local current_ports=()
    while IFS= read -r line; do
        # خطوط به فرم: listen: "0.0.0.0:1234"
        if [[ "$line" =~ listen:[[:space:]]*\"[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+:([0-9]+)\" ]]; then
            current_ports+=("${BASH_REMATCH[1]}")
        fi
    done < <(grep -E '^[[:space:]]*listen:' "$config_file")
