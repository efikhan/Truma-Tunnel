...
gre::create_interactive() {
    ...
    read -r -p "> " key
    if [[ -z "$key" ]]; then
        # Improved key generation with fallback
        if command -v openssl &>/dev/null; then
            key=$(openssl rand -hex 4 2>/dev/null | head -c8) || key=""
        fi
        if [[ -z "$key" ]]; then
            # Fallback to /dev/urandom
            key=$(od -An -N4 -tx1 /dev/urandom | tr -d ' ')
        fi
        # Ensure key is numeric and within 0-9999
        key=$((0x${key} % 10000))
    ...
    local MTU_VALUE=""
    while true; do
        echo -e "${YELLOW}Enter MTU (576-1600) [default 1400]:${NC}"
        read -r -p "> " mtu_input
...
EOF
    ...
    if systemctl enable --now "${name}.service" >/dev/null 2>&1; then
        ...
        add_log "Service failed to start."
    fi
